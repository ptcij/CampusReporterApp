...
var AddStoryView = Backbone.View.extend({
  ...
  render: function() {
    var topics = CampusReporter.topics.toJSON();
    var topic = this.model.get('topic');

    // translate
    _.each(topics, function(t) {
      t.name = CampusReporter.polyglot.t('topics.' + t.id + '.name');
    });
  ...
  create: function(topic) {
    this.model.set('id', CampusReporter.newStoryId());
    CampusReporter.stories.add(this.model);
