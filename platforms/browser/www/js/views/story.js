...
var QuestionView = Backbone.View.extend({
  ...
  render: function() {
    var q = this.question;
    q.question = CampusReporter.polyglot.t('topics.' + this.story.get('topic') + '.questions.' + q.key);
...
var StoryView = Backbone.View.extend({
  ...
  initialize: function() {
    this.topic = CampusReporter.topics.get(this.model.get('topic'));
  ...
  deleteStory: function(e) {
    ...
    if (confirm(CampusReporter.polyglot.t('story.confirm_delete'))) {
      CampusReporter.stories.remove(this.model);
      CampusReporter.trackEvent('story', 'delete');
      router.navigate('', {trigger: true});
    }
  },

  rename: function(e) {
    var title = prompt(CampusReporter.polyglot.t('story.rename'), this.model.get('title'));
